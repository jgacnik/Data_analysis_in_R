...
[First argument]{style="color: grey;"}: data frame, [second argument]{style="color: grey;"}: logical test
:::
...
[First argument]{style="color: grey;"}: data frame, [next arguments]{style="color: grey;"}: variables from the data frame for which a distinct combination is wanted
:::
...
[First argument]{style="color: grey;"}: data frame, [next arguments]{style="color: grey;"}: variables whose unique combinations are counted
:::
...
[First argument]{style="color: grey;"}: data frame, [second argument]{style="color: grey;"}: integers of the row location.
:::
...
[First argument]{style="color: grey;"}: data frame, [second argument]{style="color: grey;"}: name-value pair
:::
...
[First argument]{style="color: grey;"}: data frame, [next arguments]{style="color: grey;"}: names of columns to keep or remove
:::
...
[First argument]{style="color: grey;"}: data frame, [next arguments]{style="color: grey;"}: "new_name" = old_name, "new_name_2" = old_name_2, ...
:::
...
::: {style="font-size: 85%;"}
Function <kbd>group_by()</kbd> *silently* creates grouping of data by a specified variable
...
::: {style="font-size: 85%;"}
Function <kbd>summarise()</kbd> then collapses many rows into single summary value *per group*, the summary statistic can be mean, median, quartiles, etc.
...
Key <kbd>dplyr</kbd> functions ([cheat sheet](https://evoldyn.gitlab.io/evomics-2018/ref-sheets/R_data-transformation.pdf)):

::: {.fragment style="font-size: 80%;"}
- <kbd>filter()</kbd> – choose rows based on logic operations
- <kbd>select()</kbd> – choose columns based on column names
- <kbd>mutate()</kbd> – create new columns or modify existing ones using calculations or functions
- <kbd>group_by()</kbd> + <kbd>summarise()</kbd> – aggregate by group to create summary statistics
- <kbd>%>%</kbd> – chain steps into a pipeline
:::

# Next up

::: {style="font-size: 90%;"}
Transformed data still needs the right *shape* to work well with <kbd>ggplot2</kbd>, <kbd>dplyr</kbd>, and other common R functions.
