...
::: {style="font-size: 80%;"}
- **Numerical** variables can be discrete or continuous. For continuous numerical variables, *histograms* are commonly used (<kbd>geom_histogram()</kbd>).
- Alternative to a histogram is a *density plot* (<kbd>geom_density()</kbd>)
:::
...
- Faceting by a *single variable* uses <kbd>facet_wrap()</kbd> — specify the variable with <kbd>\~</kbd> followed by its name.
...
::: {.fragment data-fragment-index="4" style="font-size: 65%;"}
The x axis text overlaps, homework - try solving this by adjusting <kbd>theme()</kbd> parameters
:::
...
::: {.fragment data-fragment-index="1" style="font-size: 65%;"}
- Faceting by *two variables* is done using <kbd>facet_grid()</kbd> — specify the 2 variables using the formula <kbd>variable1 \~ variable2</kbd>.
:::
...
::: {.fragment data-fragment-index="4" style="font-size: 65%;"}
- The <kbd>NA</kbd> column on the plot indicates entries with missing sex data, we have to **filter** them out

- Filtering part of next presentation - **data transformation**
:::
:::::
...
Multiple ways to do it, commonly done using the <kbd>ggsave()</kbd> function
...
::: {.fragment data-fragment-index="1" style="font-size: 75%;"}
- Specifying <kbd>width</kbd>, <kbd>height</kbd>, and <kbd>units</kbd> arguments will make your plot reproducible
:::
...
::: {.fragment data-fragment-index="2" style="font-size: 75%;"}
- Without specifying dimensions, <kbd>ggsave()</kbd> will guess the size based on current size of your graphics device
:::
...
![](figures/example_barplot.png){fig-align="center" width="58%"}
...
![](figures/example_map.png){fig-align="center" width="55%"}
:::
...
![](figures/example_basketball.png){fig-align="center" width="55%"}
:::

# Next up

::: {style="font-size: 90%;"}
Plots are only as good as the data behind them. Next, we'll learn to reshape and summarize data so it is ready to plot.
:::

::: {.fragment style="font-size: 100%; margin-top: 1em;"}
**Part 3 – Data transformation** with <kbd>dplyr</kbd>
:::
